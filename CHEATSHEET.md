...
| キー | モード | 機能 | 説明 |
|------|--------|------|------|
| `<leader>ss` | Normal | シンボルへジャンプ | LSPシンボルへジャンプ |
| `<leader>sw` | Normal | ワークスペースシンボル | ワークスペース全体のLSPシンボル |

**コマンド:**
- `:Namu symbols` - シンボル検索
- `:Namu workspace` - ワークスペース全体のシンボル検索
...
| キー | モード | 機能 | 説明 |
|------|--------|------|------|
| `<space>m` | Normal | 複数行に展開 | 構造化されたコードを複数行に展開 |
...
```vim
:OverseerRun       " タスク実行
:OverseerToggle    " タスクリストの表示切り替え
:OverseerInfo      " Overseer情報表示
```

### 悪い習慣矯正（Hardtime）

繰り返しの`h`、`j`、`k`、`l`キーの使用を制限し、より効率的な移動コマンドの使用を促します（自動動作）。
