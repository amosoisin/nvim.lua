...
4. [Git操作](#git操作)
5. [ファイル操作](#ファイル操作)
6. [診断・トラブルシューティング](#診断トラブルシューティング)
7. [プラグイン管理](#プラグイン管理)
8. [その他の便利機能](#その他の便利機能)
...
```vim
:Mason             " Masonダッシュボードを開く
:MasonInstall <name>  " ツールをインストール
:MasonUpdate       " インストール済みツールを更新
:LspInfo           " LSP接続状態を確認
```
...
Hopプラグインは有効ですが、デフォルトのキーマップは設定されていません。
コマンドで使用：
...
**LSP設定について:**
このプロジェクトは**Neovim 0.11+のビルトインLSP機能**を使用しています。Masonは`none-ls`等のツールインストール用に残していますが、LSPサーバーの管理には使用していません。詳細は[README.md](README.md)または[CLAUDE.md](CLAUDE.md)を参照してください。
